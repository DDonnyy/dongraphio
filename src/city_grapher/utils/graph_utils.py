...
def _parse_overpass_route_response(loc, city_crs, boundary):
...
def _project_platforms(loc, city_crs):
...
def _recursion(stops, threshold):

    stops["to_del"] = stops.apply(
        lambda x: _get_index_to_delete(stops, x, threshold), axis=1
    )
...
def _get_index_to_delete(other_stops, loc_stop, threshold):
...
def _get_line_from_start_to_end(line, line_length):
...
def _get_nearest_edge_geometry(points, G):
...
def _project_point_on_edge(points_edge_geom):
...
def _update_edges(points_info, G):

    G_with_drop_edges = _delete_edges(points_info, G)
    updated_G, split_points = _add_splitted_edges(G_with_drop_edges, points_info)
    updated_G, split_points = _add_connecting_edges(updated_G, split_points)
...
def _delete_edges(project_points, G):
    ...
    bunch_edges = []
    G_copy = _convert_geometry(G.copy())
...
def _check_parallel_edge(G, u, v, n):
    if u == v:
        return 1
...
def _add_splitted_edges(G, split_nodes):
    ...
    nodes_bunch = split_nodes.apply(lambda x: _generate_nodes_bunch(x), axis=1)
    nodes_attr = (
...
def _add_connecting_edges(G, split_nodes):
